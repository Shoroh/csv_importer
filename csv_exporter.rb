...
class CsvExporter
  ...
  def self.transfer_and_import(send_email = true)
    ...
    Net::SFTP.start(@sftp_server, "some-ftp-user", :keys => ["path-to-credentials"]) do |sftp|
      ...
      sftp_entries.each do |entry|
        ...
        # Bang! locks the process.
        # It's better to download all files at once with download & wait
        sftp.download!(file_remote, file_local)
        # Why do we remove the file with the .start extension?
        # Where did we get it at all?
        ...
        if result == 'Success'
          ...
          # Why break? Let's continue with the rest files!
          break
        end
      end
    end
  ...
  def self.import_file(file, validation_only = false)
    ...
    success_rows = []
    import_rows = CSV.read(file, { :col_sep => ';', :headers => true, :skip_blanks => true } ).map{ |r| [r.hash['ACTIVITY_ID'], r.to_hash] }
